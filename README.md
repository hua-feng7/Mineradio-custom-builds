...
- 基于节奏的电影镜头视觉系统
- 面向长播客和 DJ 曲目的专属视觉模式
- 歌词舞台、自定义歌词、歌词位置与视觉控制；默认 15 行、三方逐字歌词、优化（SDF/MSDF）加载方式和 `-0.4` 秒换句时差
- 自定义专辑封面上传与裁剪
...
Windows 用户下载安装包后直接运行安装。Android 平板/手机使用手动构建的 APK 安装包，不走应用内更新。
...
桌面版入口由 Electron 主进程加载本地服务。`pnpm run build:win` 会生成 Windows NSIS 安装包，产物位于 `dist/`。

## 分发与更新方式

当前版本已移除应用内自动更新入口、更新检测接口、快速补丁接口和自动打开安装包的桌面 IPC。后续更新采用手动分发：Windows 发布新的 `Mineradio-*-Setup.exe`，Android 发布新的时间戳 APK。用户自行下载安装新版，不依赖 `latest.yml` 或快速补丁 JSON。
